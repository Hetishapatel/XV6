...
extern void increfcount(uint64 pa);
...
//************************************
// maps all the parent's physical pages into child instead of allocating new pages
// update parent's flags to clear PTE_W and set PTE_COW
// copy flags of parent to child flags
// increment the refcount
// map the child pages to same pa pages as of parent's
// unmap if any error
//*************************************
int
...
void vmprintwalk(pagetable_t pagetable, int level)
{
  ...
  // there are 2^9 = 512 PTEs in a page table. so we check all the pages.
  // for(int i = 0; i < 512; i++){
  //   pte_t pte = pagetable[i];
